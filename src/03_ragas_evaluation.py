...
def main():
    ...
    if report_path.exists():
        print("\n⚡ Tìm thấy kết quả đã chấm, in ra màn hình trực tiếp để tiết kiệm thời gian...")
        report = json.loads(report_path.read_text(encoding="utf-8"))
        v1_scores = report["prompt_v1_scores"]
        v2_scores = report["prompt_v2_scores"]
    else:
    ...
    for metric in ["faithfulness", "answer_relevancy", "context_recall", "context_precision"]:
        s1, s2  = v1_scores.get(metric, 0), v2_scores.get(metric, 0)
        import math
        # Xử lý NaN
        s1_val = 0.0 if math.isnan(s1) else s1
        s2_val = 0.0 if math.isnan(s2) else s2
        winner  = "← V1" if s1_val > s2_val else "← V2"
        print(f"  {metric:30s}  {s1_val:>8.4f}  {s2_val:>8.4f}  {winner}")

    # Kiểm tra mục tiêu (fix lỗi NaN > 0.8 = False)
    v1_faith = v1_scores.get("faithfulness", 0.0)
    v2_faith = v2_scores.get("faithfulness", 0.0)
    v1_faith = 0.0 if math.isnan(v1_faith) else v1_faith
    v2_faith = 0.0 if math.isnan(v2_faith) else v2_faith
    best_faith = max(v1_faith, v2_faith)
